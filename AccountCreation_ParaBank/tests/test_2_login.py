...
def get_login_data():  # Method to fetch the Login data from the Excel sheet
    ...
    path = os.path.abspath("data/login_data.xlsx")
    df = pd.read_excel(path, 'Login')  # Update the path to your Excel file if needed
...
def update_excel(username, password, test_result, tester_name):  # Method to update the status in the Excel sheet
    ...
    file_path = os.path.abspath("data/login_data.xlsx")
    temp_file_path = os.path.abspath("data/login_data_temp.xlsx")
...
class TestLogin:
    ...
    def test_login(self, setup, username, password):
        ...
        if "overview.htm" in self.driver.current_url:
